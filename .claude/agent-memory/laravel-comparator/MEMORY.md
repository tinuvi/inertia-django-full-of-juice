...
- **2026-07-16: `3.x` MOVED — `Middleware.php` last touched 2026-07-02 by `022d54f` (PR #884), now 251 lines (was 249). `onVersionChange` L207-219 now sets `Header::VERSION` at L216. `ResponseFactory::getVersion` shifted to L154-160. Client `inertiajs/inertia@3.x` `packages/core/src/response.ts` L209-222 touched 2026-07-02 (PR #3180). Anything reviewed against pre-2026-07-02 readings of the version path is stale.**
...
- [X-Inertia-Version on the 409](version_header_on_409.md) — Middleware:216 (PR #884→v3.1.1) gates client v3.6.0 async guard; fix = force_refresh only, ALWAYS set. Adversarially re-verified 2026-07-16.
- [Asset version handling](asset_version_handling.md) — Laravel getVersion(): string at ResponseFactory:154-160; Django's cast bug FIXED by resolve_inertia_version() — now MIRRORED.
