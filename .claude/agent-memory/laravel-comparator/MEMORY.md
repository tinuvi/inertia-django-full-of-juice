...
- [Validation errors / error bags](validation_errors.md) — Laravel Middleware:68-73,223-247 vs Django _resolve_session_errors/flash_errors: NOW MIRRORED (pull, first-message, header nesting); gaps = named bags, replace-vs-merge corner.
- [flash + rescuedProps page fields](flash_and_rescued_props.md) — Laravel `flash` + `rescuedProps` mechanics; Django NOW MIRRORS both (http.py flash()/build_flash/_rescued_props).
