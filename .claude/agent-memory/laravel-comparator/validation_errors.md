...
- L817-837 `flash_errors()` (storage half, mirrors `withErrors`): normalizes Form/dict → `{field: [str]}`; MERGES with existing (`{**existing, **normalized}` L833).
...
2. Two failed POSTs before any GET: Laravel REPLACES the default bag; Django MERGES field-wise (second wins per-field). Observable only in that interleaving.
