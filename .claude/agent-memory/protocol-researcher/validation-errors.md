...
- React (3.x): NO form-level errorBag on useForm — pass per-submit: `form.post(url, {errorBag: 'x'})` (submit options are VisitOptions; Visit.errorBag: string|null at core types.ts L323). `<Form errorBag="x">` IS a component prop (react/src/Form.ts L59 default null → submitOptions L228). Docs: validation.mdx Error Bags shows router.post(url, data, {errorBag}); "If you're using the form helper, it's not necessary to use error bags since validation errors are automatically scoped to the form object making the request."
- React useForm onError = clearErrors() then setError(scopedErrors) → form.errors REPLACED with exactly the scoped bag contents (useForm.ts ~L200). hasErrors = Object.keys(local errors).length > 0 (useFormState.ts L349) — per-form local state only. markAsSuccessful clears that form's errors on success (useFormState.ts L300). Other forms' local errors untouched by another form's submit (their callbacks never fire); page state preserved by default on post/put/patch/delete (validation.mdx L160).
- Mis-pairing fails SILENT: flat server errors + client errorBag set ⇒ onError still fires (top-level errors non-empty) but with {} (getScopedErrors: errors[bag] || {}) ⇒ nothing displayed, hasErrors false.

## PRG flow with error bags (Laravel reference)
- Client sends ONE XHR; the 302 is followed by the browser HTTP stack, which re-sends request headers — so X-Inertia-Error-Bag IS present on the followed GET. Laravel test simulates exactly this: `get('/', ['X-Inertia-Error-Bag' => 'example'])` asserting `$errors->example->email` (inertia-laravel 3.x tests/MiddlewareTest.php).
- Laravel does NOT persist the bag name in the session. Middleware::resolveValidationErrors (src/Middleware.php ~L222-248) reads session-flashed errors + the CURRENT (GET) request's header: if session bag is 'default' AND header present ⇒ re-key as `[header => defaultBag]`; if 'default' only ⇒ flat; else (validateWithBag named bags) ⇒ `$bags->toArray()` nesting under session bag names. Shared via `'errors' => Inertia::always(resolveValidationErrors())` (L68-73).
- Client-side scoping after PRG works because response.ts getScopedErrors reads requestParams of the ORIGINAL visit (the POST's errorBag) against the final (post-redirect GET) page props.
