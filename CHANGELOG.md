...
### Added
- `inertia.precognition` module with a `@precognition(FormClass, *, form_kwargs=None)` view decorator and `is_precognitive(request)` / `validate_only_keys(request)` helpers (both exported from `inertia`), implementing the v3 Precognition wire contract: precognitive requests (`Precognition: true`) are answered without running the view body — `204 No Content` + `Precognition-Success: true` on success, `422` `{"message", "errors": {field: [messages]}}` on failure — every response gains `Vary: Precognition` (via `patch_vary_headers`), and precognitive responses echo `Precognition: true` (the v3 client throws without it). `Precognition-Validate-Only` scoping pops unlisted fields off the form instance before validation (with Laravel's `*` = one-dot-segment wildcard semantics); popped fields are absent from `cleaned_data` — cross-field `clean()` must use `.get()` (the documented Django contract) and `add_error()` on a popped field raises. Bodies are parsed as the client sends them (JSON by default, query params for GET/DELETE, multipart/urlencoded for POST and non-POST alike; non-POST multipart goes through `request.parse_file_upload`, the same parse and upload-handler freeze as Django's own POST path). `form_kwargs` — a per-request callable mirroring Django's `FormMixin.get_form_kwargs` — is splatted into the form constructor, unblocking `SetPasswordForm(user, …)`-style forms and ModelForm `instance` update flows. Sync views only — the shipped library is synchronous by design (async deployments are served by gevent): handing the decorator an `async def` view raises `TypeError` at decoration time (`asgiref.sync.iscoroutinefunction` guard), and ORM-touching validation (`ModelForm.validate_unique`, `ModelChoiceField` querysets) runs on the ordinary synchronous request path. `method_decorator`-compatible, with a `_check_request`-style TypeError guard (the `django/views/decorators/cache.py` idiom) when the first argument isn't a request. In Laravel this surface lives in `laravel/framework` (`HandlePrecognitiveRequests`, `Foundation/Precognition.php`, the precognition dispatchers), not `inertiajs/inertia-laravel` — the decorator is the Django mirror of the per-route `precognitive` middleware alias. Intentional divergence: an unparseable envelope (broken JSON, non-object JSON, corrupt multipart) returns our JSON `400` `{"message": "Malformed request body."}` with the `Precognition: true` echo, following Django core's explicit-400 stance for unreadable envelopes (`MultiPartParserError` → 400 in `django/core/handlers/exception.py`) instead of Laravel's coerce-to-empty → `422` (JSON via `Request::json`'s swallowed decode failure; multipart via PHP's native form parsing). The client has no `400` status hook, so the echo doesn't make it handle the `400` — it makes the rejection truthful (`HTTP error 400` with the JSON body on `error.response.data`) instead of the misleading missing-middleware error.
- `flash(request, /, **kwargs)` and the v3 `flash` page-object field: one-shot data merged into the session (`_inertia_flash`) and pulled at render time on every Inertia render, partial reloads included — mirroring `inertiajs/inertia-laravel@3.x` `ResponseFactory::flash()` / `Response::resolveFlashData()` (session key `inertia.flash_data`, pull semantics). Omitted when empty. `request` is positional-only, so a flash key literally named `request` stays usable. In mixed Inertia/classic apps, flashed state waits in the session for the next Inertia render (bounded by session expiry, the same as unread `contrib.messages`).
- `INERTIA_FLASH_FROM_MESSAGES` setting (default `False`): when enabled, each render drains `django.contrib.messages` (iterating the storage marks it used — the load-bearing mutation that lets `MessageMiddleware`'s exit path clear the store) into `flash["messages"]` as `{message, level, tags, extra_tags, level_tag}` dicts. Lazy translation proxies (`gettext_lazy` messages / `extra_tags`) are str-coerced during the drain, and `tags` is computed over the coerced values replicating `Message.tags`' join semantics (`django/contrib/messages/storage/base.py`) — reading `message.tags` raw would feed the proxy into `str.join` and raise `TypeError`. Render-time draining means partial reloads and multi-hop redirect chains can no longer consume a pending message without delivering it — the failure modes of the eager per-request middleware recipe.
- Built-in validation-errors flow: `flash_errors(request, errors)` (accepts a bound `django.forms` form or a `{field: message(s)}` mapping, normalized to lists of strings — `ValidationError` values flatten to their message strings via the `Form.add_error` idiom, and empty-list values are dropped at store time) stores one-shot errors in the session (`_inertia_errors`); each call REPLACES any errors already flashed, matching Laravel's `redirect()->withErrors()` replace semantics — Django form errors are a per-run snapshot of the whole submission, so stale fields from an earlier flash never resurface. The next render pulls them into the `errors` prop with first-message-per-field flattening and `X-Inertia-Error-Bag` nesting, mirroring `inertiajs/inertia-laravel@3.x` `Middleware::resolveValidationErrors`; non-field errors arrive under Django's literal `__all__` key (matching `errors.get_json_data()`). Shared or per-render `errors` props win over the session flow (and the session bag still ages out), so pre-0.5.0 hand-wired recipes keep working unchanged. Intentional divergence: errors are stored as a single default bag (Django has no `ViewErrorBag`), so bag nesting is driven purely by the request header.
- `redirect_back(request, *, errors=None, fallback="/")` — redirect-to-referrer helper mirroring Laravel's `Inertia::back()` + `redirect()->withErrors()` pairing. Intentional divergence: the `Referer` is validated with Django's `url_has_allowed_host_and_scheme` (the `contrib.auth` idiom) before use, so the attacker-controlled header can never produce an open redirect; unsafe or missing referrers use `fallback`, resolved through `django.shortcuts.resolve_url` so URL names work alongside literal paths.
- `defer(..., rescue=True)` and the v3 `rescuedProps` page-object field, mirroring `inertiajs/inertia-laravel@3.x` `ResponseFactory::defer($callback, $group, $rescue)` and `PropsResolver::resolveValue()`: when a rescuable prop's resolver raises, the exception is reported (here via `logger.exception` on the `inertia_django_full_of_juice` logger — the Django mirror of Laravel's `report()`), the prop is dropped from `props`, and its key is emitted in `rescuedProps` (only when non-empty). Rescued props also drop their merge metadata — `mergeProps` / `prependProps` / `deepMergeProps` / `matchPropsOn` never advertise a value that was dropped, mirroring how the rescued `continue` precedes `collectMetadata` in Laravel's `PropsResolver::resolveProps`. The check is duck-typed on `should_rescue()` like Laravel's `Rescuable` interface, so custom prop classes can opt in.
